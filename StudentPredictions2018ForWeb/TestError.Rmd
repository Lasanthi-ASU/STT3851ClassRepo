...
#names(MurrayLauren) <- "WidmanSamantha"
#
#Wilson_Courtney  <- read.csv("Wilson_Courtney.csv")
#WilsonCourtney <- sqrt(mean((Wilson_Courtney$x - APRICE$price)^2, na.rm = TRUE))
...
```{r, echo = FALSE}
Leader <- data.frame(SMSPE = c(BeatThis, DoeJohn,BrainJessica,VickMaya,TsraManuela,FisherTameshia,FoxMichael,MitchellEmily,SmithAbigail))
```
